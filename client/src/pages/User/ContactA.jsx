...
const ContactA = () => {
  ...
  const fetchUserMessages = async () => {
    if (!userId) return;
    try {
      const res = await axios.get(`${API_URL}/api/message/user/${userId}`);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      await axios.post(`${API_URL}/api/message`, { question, examineeId: userId });
      setQuestion('');
      fetchUserMessages();
    } catch (err) {
  ...
  const editMyMessage = async (id, currentText) => {
    ...
    try {
      await axios.put(`${API_URL}/api/message/edit/${id}`, {
        question: newText,
        role: 'user',
        userId
      });
